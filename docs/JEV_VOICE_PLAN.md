...
- Deliberately deferred:
  ...
  - A wake word. Push-to-talk is simpler and more private. **2026-09-25:** the key is now tap-to-talk by default (recording ends on a second of quiet), with `start_voice.bat` and `--autostart on` to skip the terminal. That was the user's pick of four ideas to cut effort; a system-tray icon and a local wake word (openWakeWord) are the next two, not built. **Later 2026-09-25:** the tray icon is built, with a floating status window whose Yes / No buttons sit next to the spoken confirmation (`voice_ui.py`), after a look at how Aemyos (Windows) and Samuel (macOS) present theirs. The wake word is still not built. **Then:** a full app window (`app_ui.py`, pywebview) with a step feed, history, Pause / Take over / Stop, compact mode, messages mid-task and the agent asking questions (`ask_user`), after a look at Magentic-UI, ChatGPT agent and UI-TARS Desktop.
