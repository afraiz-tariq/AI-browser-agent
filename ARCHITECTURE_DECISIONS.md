...
2. **MCP as a third `ToolProvider`.** Additive, not a replacement for the native arms. One read-only server first (web search is the safest proving ground) to validate the mechanism end-to-end before adding more. The agent's own policy engine — not the MCP server's self-description — decides risk tier; an MCP server's own new/future tools default to `R3` (confirm) until explicitly classified, so a server update can't silently introduce an unreviewed dangerous tool.
